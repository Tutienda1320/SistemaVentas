...
public class Fcompra {
    ...
    public DefaultTableModel mostrar() {
        ...
        String[] titulos = {"Cod Compra","Cod proveedor",
        ...
        sSQL = "select c.cod_comp,"
                ...
                + "on c.cod_prov = p.cod_prov order by c.fecha_comp DESC ";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                registros[2] = rs.getString("nombre_prov");
                registros[3] = rs.getString("numfactura_comp");
                registros[4] = rs.getString("fecha_comp");
                registros[5] = rs.getString("total_comp");
    ...
    public boolean insertar(Dcompra datos) {
        ...
        sSQL = "insert into compras "
                + "(cod_prov,numfactura_comp,fecha_comp,total_comp)"
                + "values(?,?,?,?)";
        ...
        try {
            ...
            pst.setInt(1, datos.getcod_prov());
            pst.setString(2, datos.getnumfactura_comp());
            pst.setDate(3, datos.getfecha_comp());
            pst.setInt(4, datos.gettotal_comp());
    ...
    public int consultaCodigo(String numFactura){
        int Codigo=0;
        sSQL = "select cod_comp from compras where numfactura_comp ="+numFactura;
        try{
            Statement st = cn.createStatement();
            ResultSet rs = st.executeQuery(sSQL);
